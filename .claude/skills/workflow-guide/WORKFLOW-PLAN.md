...
승인 후 MEMORY.md의 `# currentWork` 섹션을 아래 형식으로 업데이트한다:
```
# currentWork
- 이슈: #{번호} {제목}
- 브랜치: {브랜치명}
- 단계: Phase 3 완료 — 구현 진행 중
- 변경 범위:
  - `{파일경로}`: {변경 내용}
  - `{파일경로}`: {변경 내용}
```

> **단순 작업은 /clear 없이 연속 진행을 권장한다.**
> 파일로 저장된 계획 산출물이 없으므로 /clear 시 계획 컨텍스트가 소멸한다.
> 컨텍스트 절감 효과보다 재작업 비용이 크다.
...
## [세션 격리 체크포인트 A — 복잡 작업 전용]

> **적용 조건: 구현계획.md와 탐색결과.md가 모두 작성된 복잡 작업에만 해당.**
> 단순 작업(파일 < 5개)은 /clear 없이 연속 진행한다.
>
> **Phase 3 완료 & 사용자 승인 직후:**
>
> ```
> 1. .claude/workspace/구현계획.md 내용을 최종 확인한다
> 2. /clear 로 대화 이력을 초기화한다
> 3. 새 세션 시작 후 아래 순서로 컨텍스트를 복원한다:
>    a. workflow-guide 스킬 로드 → WORKFLOW-IMPL.md 읽기
>    b. coding-rules 스킬 로드 → 작업 유형에 맞는 규칙 파일 읽기
>    c. .claude/workspace/구현계획.md 읽기
>    d. .claude/workspace/탐색결과.md 읽기
>    e. 구현계획.md의 "변경 범위"에 있는 기존 파일 필요 시 재독
> 4. Phase 4 코드 구현 시작
> ```
>
> **복원 후 유지되는 정보**: 변경 파일 목록, 구현 순서, 설계 결정, 아키텍처 검증 체크리스트, 커밋·브랜치 계획.