...
public record BillInfoBatchEntity(
	int totalCount,
	...
	public BillInfoBatchEntity filterNormalBills() {
		List<BillInfoItem> chairmanBills = items.stream()
			.filter(item -> !item.isChairman())
			.toList();
	...
	public BillInfoBatchEntity filterChairmanBills() {
		List<BillInfoItem> chairmanBills = items.stream()
	...
	public BillInfoBatchEntity filteredBillsWithFewProposers() {
		List<BillInfoItem> billsWithFewProposers = items.stream()
			.filter(BillInfoItem::isProposerLessThanTwentyOrChairman)
			.toList();
		return new BillInfoBatchEntity(
			billsWithFewProposers.size(),
			billsWithFewProposers
		);
	}

	public BillInfoBatchEntity filterHasLegislationBodyBills() {
		List<BillInfoItem> hasLegislationBodyBills = items.stream()
			.filter(BillInfoItem::isNotReceivedAndWithdrawn)
			.toList();
		return new BillInfoBatchEntity(
			hasLegislationBodyBills.size(),
			hasLegislationBodyBills
		);
	}
	...
	public record BillInfoItem(
		...
		boolean isNotReceivedAndWithdrawn() {
			return progressStatus.isNotReceived()
				&& progressStatus.isNotWithdrawn()
				&& progressStatus.isNotAbrogate();
		}
