...
public record NotificationCenterResponse(List<Item> items) {
	record Item(String topic, String title, String body, String iconUrl, String billId, LocalDateTime createdAt) {
		static Item from(NotificationCenterItem item) {
			return new Item(
				item.notificationTopic().getValue(), item.title(), item.body(),
				Constant.IMAGE_ACCESS_URL + item.notificationTopic().getIconPath(), item.billId(), item.createdAt());
		}
	}
}
