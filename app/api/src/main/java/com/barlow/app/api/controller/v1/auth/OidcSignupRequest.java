...
public record OidcSignupRequest(OidcPayload oidcPayload, Map<Long, Boolean> termAgreements,
	...
	public MemberCreateCommand.UserPayload toPayload() {
		return new MemberCreateCommand.UserPayload(
			DeviceOs.valueOf(signupPayload.deviceOs().toUpperCase()), signupPayload().deviceId(),
			signupPayload().deviceToken(), signupPayload.nickname());
	}

	public MemberCreateCommand toCommand(Function<OidcAuthenticationRequest, ExternalPrincipal> authenticator,
		LocalDateTime submittedAt) {
		return new MemberCreateCommand(
			authenticator.apply(oidcPayload.toAuthenticationRequest()), toPayload(), toTermAgreementList(submittedAt));
	}
