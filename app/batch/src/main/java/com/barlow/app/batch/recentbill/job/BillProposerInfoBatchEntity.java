...
public record BillProposerInfoBatchEntity(List<BillProposerInfo> billProposerInfos) {

	public static BillProposerInfoBatchEntity from(ItemResponseBody<BillPetitionMemberListItem> memberList) {
		return new BillProposerInfoBatchEntity(
			memberList.items().stream().filter(Objects::nonNull).map(BillProposerInfo::from).toList());
	}

	public record BillProposerInfo(String name, String partyName) {
