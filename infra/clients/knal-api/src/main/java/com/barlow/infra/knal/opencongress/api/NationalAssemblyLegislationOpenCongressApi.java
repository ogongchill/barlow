...
@FeignClient(value = "${knal.open-congress.api.name}", url = "${knal.open-congress.api.url}", configuration = OpenCongressConfig.class)
public interface NationalAssemblyLegislationOpenCongressApi {

	@GetMapping(value = Operation.GET_PRE_ANNOUNCEMENT, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
