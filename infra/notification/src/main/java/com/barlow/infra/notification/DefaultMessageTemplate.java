...
public class DefaultMessageTemplate implements MessageTemplate {
	...
	public String getMessageBodyFormat(NotificationInfo.Topic topic) {
		if (topic.isCommitteeType()) {
			return String.format(
				Default.COMMITTEE_RECEIVED.bodyFormat, topic.getTopic().getValue(), topic.getRepresentation(),
				topic.getCount());
		}
		return String.format(
			Default.findByValue(topic.getTopic().getValue()).bodyFormat, topic.getRepresentation(), topic.getCount());
	}
	...
	enum Default {
		...
		SUBMISSION_PLENARY_SESSION(NotificationTopic.SUBMISSION_PLENARY_SESSION.getValue(),
			DEFAULT_MESSAGE_TITLE_FORMAT, "%s 등 %d 개의 법안이 지금 본회의에서 검토중이에요."),
		RESOLUTION_PLENARY_SESSION(NotificationTopic.RESOLUTION_PLENARY_SESSION.getValue(),
			DEFAULT_MESSAGE_TITLE_FORMAT, "%s 등 %d 개의 법안이 오늘 본회의에서 통과되었어요."),
		RECONSIDERATION_GOVERNMENT(NotificationTopic.RECONSIDERATION_GOVERNMENT.getValue(),
			DEFAULT_MESSAGE_TITLE_FORMAT, "정부에서 %s 등 %d 개의 법안에 대해 재의를 요구했어요."),
		PROMULGATION(NotificationTopic.PROMULGATION.getValue(), DEFAULT_MESSAGE_TITLE_FORMAT, "%s 등 %d 개의 법안이 공포되었어요."),

		COMMITTEE_RECEIVED("소관위접수", "%s에서 알려드려요", "%s에서 %s 등 %d개의 법안을 새롭게 검토하고 있어요."),;
		...
		static Default findByValue(String topicValue) {
			return Arrays.stream(Default.values()).filter(v -> v.value.equals(topicValue)).findFirst()
