...
public record NotificationResult(Map<Message, MessagingErrorCode> result) {
	boolean hasRetryableFailure() {
		return result.entrySet().stream()
			.filter(
				entry -> entry.getValue().equals(MessagingErrorCode.INTERNAL)
					|| entry.getValue().equals(MessagingErrorCode.UNAVAILABLE)
					|| entry.getValue().equals(MessagingErrorCode.QUOTA_EXCEEDED))
			.map(Map.Entry::getKey).findAny().isPresent();
	}
