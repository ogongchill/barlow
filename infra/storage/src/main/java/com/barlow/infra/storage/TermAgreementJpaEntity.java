...
public class TermAgreementJpaEntity extends BaseTimeJpaEntity {
	...
	public TermAgreementJpaEntity(Long memberNo, Long termNo, boolean agreed, LocalDateTime agreedAt) {
	...
	public static List<TermAgreementJpaEntity> fromCommand(UserTermAgreementCommand command) {
		return command.agreements().stream()
			.map(
				agreement -> new TermAgreementJpaEntity(
					command.userNo(), agreement.termId(), agreement.agreed(), agreement.agreedAt()))
			.toList();
	}
