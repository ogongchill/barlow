...
public class CommitteeSubscriptionController {
	...
	public ApiResponse<CommitteeSubscriptionNotificationResponse> retrieveCommitteeAccountSubscribeInfo() {
		// 사용자 불러오기 추가
		Long memberNo = null;
		List<CommitteeSubscriptionNotification> committeeNotificationSubscriptions = getCommitteeSubscriptionNotifications(
			memberNo);
	...
	private List<CommitteeSubscriptionNotification> getCommitteeSubscriptionNotifications(Long memberNo) {
		...
		MemberNotificationConfig notificationConfig = notificationConfigRetrieveService.retrieveMemberNotificationConfig(
			memberNo);
