...
public class TestKeyConfig {
	...
	public Algorithm privateKeyAlgorithm() {
		try {
			return new AlgorithmConfig().jwtPrivateKeyAlgorithm(PRIVATE_KEY, RSA_KEY_FACTORY);
	...
	public Algorithm publicKeyAlgorithm() {
		try {
			return new AlgorithmConfig().jwtPublicKeyAlgorithm(PUBLIC_KEY, RSA_KEY_FACTORY);
		} catch (InvalidKeySpecException e) {
